...
class Track(AtomicDEVS):
    def __init__(self, origin: str, trolley: Optional[Trolley],
                 arriving_delay: float, unboarding_delay: float,
                 boarding_delay: float, departing_delay: float,
                 random_unboard: float):
        ...
        # "boarding": Trolley,
        # "departing": Trolley
        self.state = ["none", None] if trolley is None else ["boarding",
                                                             trolley]
        self.count = 0
    ...
    def __passenger_indices(self):
        return [idx for idx, psgr in enumerate(self.state[1].passengers)
                if
                self.origin == psgr.destination or random.random() < self.random_unboard]
    ...
    def intTransition(self):
        if self.state[0] == "arriving":
            ...
            prev_len = len(self.state[1].passengers)
            self.state[1].passengers = [p for idx, p in enumerate(self.state[1].passengers) if idx not in self.state[2]]
            assert prev_len == len(self.state[1].passengers) + len(
                self.state[2])
            ...
            logging.debug(
                f"TRACK: boarding is done: {len(self.state[1].passengers)} passengers are here, departing...")
    ...
    def extTransition(self, inputs):
        if self.board in inputs and inputs[self.board] is None:
            logging.debug(
                "TRACK: departing due to a lack of waiting passengers")
            ...
            assert passenger.destination in passenger.lines[self.state[1].line]
    ...
    def outputFnc(self):
        if self.state[0] == "none":
            ...
            return {self.depart: self.state[1].passengers[
